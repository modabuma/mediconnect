document.addEventListener("DOMContentLoaded", async function(){
    ...
    const patient = document.getElementById("filter-patient");
    ...
    const response = await fetch("http://127.0.0.1:5000/administrator/get_users?option=1&role=3", options);

    const resp_patient = await response.json();
    const resp_symptom = await response_symptom.json();

    for (let i = 0; i < resp_patient.data.length; i++){
        const option = document.createElement("option");

        option.value = resp_patient.data[i].id;
        option.text = resp_patient.data[i].additional_data.names+" "+resp_patient.data[i].additional_data.lastnames;
        ...
        for (let i = 0; i < resp_appointments.data.records.length; i++){
            ...
            if (data.length == 0){
                data.push({
                    ...
                    patient: resp_appointments.data.records[i].user.additional_data.names+" "+resp_appointments.data.records[i].user.additional_data.lastnames,
