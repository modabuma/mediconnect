document.addEventListener("DOMContentLoaded", async function(){
    ...
    const response = await fetch("http://127.0.0.1:5000/symptoms/get", options);
    ...
    if (response.status != 200){
        ...
        for (let i = 0; i < resp.data.records.length; i++){
            ...
            <tr>
                <th>
                    <button class="btn btn-danger btn-sm" onclick="deleteSymptom(`+resp.data.records[i].id+`)">
...
async function otherPage(page){
    ...
    const response = await fetch("http://127.0.0.1:5000/symptoms/get?page="+page, options);
...
async function getDataWithFilters(){
    ...
    const response = await fetch("http://127.0.0.1:5000/symptoms/get?"+filters, options);
    const resp = await response.json();
...
async function delete_(id){
    ...
    const response = await fetch("http://127.0.0.1:5000/symptoms/delete", options);
    const resp = await response.json();
...
function redirectToUpdateView(id){
    ...
    window.location.href = "../../admin/symptoms/updateSymptoms.html";
}