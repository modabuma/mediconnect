document.addEventListener("DOMContentLoaded", async function(){
    ...
    const resp_patients = await response.json();
    const resp_doctor = await response2.json();
    const resp_symptom = await response3.json();

    for (let i = 0; i < resp_patients.data.length; i++){
        const option = document.createElement("option");

        option.value = resp_patients.data[i].id;
        option.text = resp_patients.data[i].additional_data.names+" "+resp_patients.data[i].additional_data.lastnames;
...
async function getAppointments(){
    ...
    if (symptom != 0 && sub_symptom != 0 && patient != 0 && doctor != 0){
        for (let i = 0; i < 3; i++){
